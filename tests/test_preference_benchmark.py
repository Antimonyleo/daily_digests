...
embeddings (no model download), and asserts:
  ...
  1. the full model's HELD-OUT pairwise accuracy beats the topic-cosine-only
     baseline on clearly-separable liked/disliked clusters, and
  2. exemplar construction is leakage-free — no test-set item id appears in the
     train exemplar id arrays.

The prior "0.79 vs 0.74" status claim had no committed, runnable script behind
it. This test IS the runnable evidence for the mechanism; the real-DB number is
reported by `scripts/benchmark_ranker.py` and depends on live votes.
"""
...
def test_full_model_beats_topic_baseline_and_is_leakage_free(monkeypatch):
    ...
    # (1) Full model's held-out pairwise accuracy > topic-only baseline.
    assert not np.isnan(result["topic_acc"])
    assert not np.isnan(result["full_acc"])
    assert result["full_acc"] > result["topic_acc"], (
        f"full model ({result['full_acc']:.3f}) should beat topic-only "
        f"({result['topic_acc']:.3f}) on separable synthetic clusters"
    )
    # On cleanly-separated clusters the full model should be near-perfect.
