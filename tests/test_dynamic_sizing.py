...
def test_cross_encoder_gate_rescues_and_demotes(monkeypatch):
    """The cross-encoder relevance (cross_encoder_rel ∈ [-1,1]) fixes both
    directions of the bi-encoder's error: it RESCUES a genuinely on-field item
    whose cosine sits just under the floor and DEMOTES a surface-similar item
    whose cosine clears the floor but whose joint relevance is low."""
    from dailydigest.pipeline import _filter_off_topic

    _cfg(monkeypatch, min_topic_relevance=0.68, cross_encoder_gate_coeff=0.20)
    rescue = _Row(1, "research")   # cosine below floor, CE ranks it top
    demote = _Row(2, "research")   # cosine above floor, CE ranks it bottom
    neutral = _Row(3, "research")  # cosine above floor, CE at median
    feats = {
        _row_feature_key(rescue): {"topic_score": 0.66, "cross_encoder_rel": 1.0},
        _row_feature_key(demote): {"topic_score": 0.72, "cross_encoder_rel": -1.0},
        _row_feature_key(neutral): {"topic_score": 0.70, "cross_encoder_rel": 0.0},
    }
    scored = [(rescue, 0.66), (demote, 0.72), (neutral, 0.70)]
    kept = {row.id for row, _ in _filter_off_topic(scored, feats)}
    assert 1 in kept        # 0.66 + 0.20*1.0 = 0.86 >= 0.68  (rescued)
    assert 2 not in kept    # 0.72 + 0.20*(-1.0) = 0.52 < 0.68 (demoted)
    assert 3 in kept        # 0.70 + 0 = 0.70 >= 0.68


def test_cross_encoder_gate_inert_without_rel(monkeypatch):
    """When no cross-encoder relevance is present (offline / reranker
    unavailable), the gate is pure cosine — unchanged behavior."""
    from dailydigest.pipeline import _filter_off_topic

    _cfg(monkeypatch, min_topic_relevance=0.68, cross_encoder_gate_coeff=0.20)
    on = _Row(1, "research"); off = _Row(2, "research")
    feats = {
        _row_feature_key(on): {"topic_score": 0.70},
        _row_feature_key(off): {"topic_score": 0.66},
    }
    scored = [(on, 0.70), (off, 0.66)]
    kept = {row.id for row, _ in _filter_off_topic(scored, feats)}
    assert kept == {1}


def test_annotate_cross_encoder_relevance_normalizes(monkeypatch):
    """_annotate_cross_encoder_relevance maps raw CE scores to tanh z-scores
    (median→0, symmetric), only over research items, and is inert below 5."""
    from dailydigest.pipeline import _annotate_cross_encoder_relevance

    rows = [_Row(i, "research") for i in range(1, 8)]
    ces = [0.001, 0.002, 0.003, 0.004, 0.005, 0.006, 0.007]  # ascending, median=0.004
    sink = {_row_feature_key(r): ce for r, ce in zip(rows, ces)}
    feats = {_row_feature_key(r): {"topic_score": 0.70} for r in rows}
    scored = [(r, 0.70) for r in rows]
    _annotate_cross_encoder_relevance(scored, feats, sink)
    rels = [feats[_row_feature_key(r)]["cross_encoder_rel"] for r in rows]
    # Raw CE recorded, rel monotonic increasing, median item ~0, symmetric ends.
    assert all("cross_encoder_score" in feats[_row_feature_key(r)] for r in rows)
    assert rels == sorted(rels)
    assert abs(rels[3]) < 1e-6          # median → 0
    assert rels[0] < 0 < rels[-1]
    assert abs(rels[0] + rels[-1]) < 1e-6  # symmetric

    # Fewer than 5 items -> no rel annotation (percentile not meaningful).
    rows2 = [_Row(i, "research") for i in range(20, 23)]
    sink2 = {_row_feature_key(r): 0.5 for r in rows2}
    feats2 = {_row_feature_key(r): {"topic_score": 0.70} for r in rows2}
    _annotate_cross_encoder_relevance([(r, 0.70) for r in rows2], feats2, sink2)
    assert all("cross_encoder_rel" not in feats2[_row_feature_key(r)] for r in rows2)
